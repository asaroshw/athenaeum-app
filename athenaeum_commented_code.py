...
===============================
Streamlit application for sector-aware fundamental, intrinsic-value,
technical, catalyst, and AI-assisted stock analysis.

METHODOLOGY CONTRACT
--------------------
The accompanying Athenaeum Model Methodology is authoritative for financial
logic. This file implements that methodology; it should not redefine it.
If implementation and methodology disagree, the discrepancy should be
identified and resolved explicitly rather than silently changing the model.

Comments in this file explain WHY non-obvious calculations exist, especially
where thresholds, fallbacks, sector adjustments, or scoring rules are
methodology-driven.
"""
...
# ============================================================
# UI configuration, API credentials, and presentation constants live here.
# Financial-model constants are kept separately below so that methodology
# parameters are easier to audit and change deliberately.
...
# ============================================================
# TOP SECTOR PEERS
# These are fallback comparison universes used only when the analysed stock
# receives an OBSERVE/DON'T BUY result. They are not part of the core
# fundamental score and should not influence the primary verdict.
...
# 2. CORE UTILITIES
# ============================================================
# These helpers normalize provider-specific values before financial logic
# consumes them. Missing data remains missing rather than becoming a fake 0.
...
# ============================================================
# Sector classification determines which fundamental checks and valuation
# assumptions are appropriate. Financials are treated separately because
...
# ============================================================
# News is treated as a qualitative evidence layer. It can identify catalysts
# and risks, but it must not invent quantitative facts that are absent from
# the underlying data.
...
def scan_news_sentiment(recent_news, business_summary):
    ...
    NEGATION = ['denies','deny','clears','cleared','no evidence','exonerated',
                 'completed successfully','dismissed','baseless','unfounded','false report']
    titles = [n.get('title', '') for n in (recent_news or [])]
    text = " ".join(titles).lower()  # Use current news headlines rather than the static business summary for catalyst materiality.
    ...
    # Apply the risk penalty only when the headline does not contain an explicit exonerating/negating context.
    confirmed_risks = [rk for rk in risk_hits if not any(
        rk in t.lower() and any(neg in t.lower() for neg in NEGATION) for t in titles)]
    if confirmed_risks:
        bonus -= 20
        notes.append(f"Qualitative penalty (-20): confirmed risk keyword(s) in news "
                      f"({', '.join(confirmed_risks[:3])}).")
    elif risk_hits:
        notes.append(f"Risk keyword(s) appear in exonerating context — no penalty "
                      f"({', '.join(risk_hits[:3])}).")
    return bonus, notes
...
def extract_order_book_signal(recent_news, business_summary, trailing_revenue_cr=None):
    ...
    if match:
        ...
        try:
            ...
            pass
    # Order-related growth adjustments are subject to the methodology's 5% of trailing-revenue materiality threshold.
        ...
        if m_size:
            try:
                ...
                if order_cr / trailing_revenue_cr < 0.05:
                    growth_pct_found = None  # immaterial — suppress the growth bump
            except Exception:
                pass
...
# 5. CHECKLISTS
# ============================================================
# Each checklist produces an independently auditable 0-100 score. Missing
# checks are excluded from that pillar rather than being silently counted as
# passes. The final fundamental weighting is applied afterward.
...
def past_performance_checks(m):
    ...
    if yoy is not None and qoq is not None:
        # Compare YoY growth with the prior YoY period so acceleration is measured on a consistent timescale.
...
def compute_fundamental_score(val_score, past_score, health_score, is_financial):
    # Methodology §1: standard sectors use 35/35/30; financials use 45/35/20.
    # If a pillar is unavailable, its weight is renormalized across available
    # pillars so missing data does not automatically become a zero score.
...
# ============================================================
# This section contains the core financial and technical calculations.
# Methodology constants below correspond to the documented CAPM, valuation,
# technical, and composite-verdict rules.
...
def justified_pb_fair_value(roe_pct, ke_pct, growth_pct, book_value_per_share, pb_floor=0.4, pb_cap=None):
    # Financial-sector valuation is based on excess ROE: ROE above Cost of
    # Equity supports a justified premium to book value.
...
def apply_tiered_sanity_veto(verdict, target_price, current_price, notes, growth_pct=None):
    ...
    # Methodology §16: >15% modeled downside prevents a BUY verdict.
    # The additional >40% escalation to DON'T BUY is an implementation detail
    # that should remain explicitly documented if retained.
    ...
    # Extremely large implied upside is treated as a possible data/model anomaly and reviewed against verified growth.
...
def run_predictive_pipeline(info, hist, fcf_history, sector, industry, fundamental_score,
    ...
    # Methodology §3.2: CAPM Cost of Equity = Rf + Beta × ERP, with Ke
    # constrained to the 9%-20% range to prevent extreme discount rates.
    current_rfr = get_dynamic_risk_free_rate()
    ke_pct = min(max((current_rfr + beta * EQUITY_RISK_PREMIUM) * 100, 9), 20)

    # Methodology §10: growth priority is Analyst Consensus (cap 35%), then
    # trailing PAT YoY (cap 25%), then an explicit 8% model baseline.
    # The source label is retained so the UI can distinguish observed,
    # estimated, and assumed growth.
    growth_pct = 8.0
    growth_source = "default (8%)"
    if analyst_growth_pct and analyst_growth_pct > 0:
        growth_pct = min(max(analyst_growth_pct, 5), 35)  # Analyst consensus is preferred because the methodology explicitly prioritizes forward-looking EPS growth.
        growth_source = f"analyst consensus ({analyst_growth_pct:.1f}%)"
    ...
    if financial:
        ...
        if jpb_value and ddm_val:
            ...
            intrinsic_value = round(current_price * 0.70, 2)
            forced_intrinsic_adjustment = -30
            result["model_used"] = "Defensive Haircut — financial sector (no P/B or DDM inputs)"
    else:
        ...
        # High-growth earnings valuation is intentionally checked before the DCF
        # branch in the current implementation. This ordering is important: it
        # must be reconciled with the methodology's stated valuation hierarchy
        # before being changed or retained as final behavior.

        if effective_eps and effective_eps > 0 and pat_yoy_pct and pat_yoy_pct > 15:
            # Project EPS three years forward, apply the selected terminal P/E, then discount the terminal value using Cost of Equity.
            g   = min(max(pat_yoy_pct / 100, 0.05), 0.35)
            ke  = ke_pct / 100
            terminal_pe = 35 if pat_yoy_pct >= 30 else (28 if pat_yoy_pct >= 20 else 22)
            eps_y3 = effective_eps * ((1 + g) ** 3)
            intrinsic_value = round((eps_y3 * terminal_pe) / ((1 + ke) ** 3), 2)
            result["model_used"] = (f"3Y Forward EPS Compound "
                                     f"(g={round(g*100,1)}%, P/E={terminal_pe}x, ke={round(ke*100,1)}%)")
            ...
            if fcf_per_share > 0:
                ...
                intrinsic_value = None   # fall through to book/EPS chain below
                result["model_used"] = "FCF Negative — falling through to earnings/book fallback"
                
        elif effective_eps and effective_eps > 0:
            # Defensive earnings valuation still uses a three-year forward EPS framework rather than treating current EPS as terminal value.
            g   = min(max((pat_yoy_pct or 8) / 100, 0.05), 0.15)
            ke  = ke_pct / 100
            historical_pe = resolved_pe if (resolved_pe and resolved_pe > 0) else 15.0
            terminal_pe = min(max(historical_pe, 12), 20)
            eps_y3 = effective_eps * ((1 + g) ** 3)
            intrinsic_value = round((eps_y3 * terminal_pe) / ((1 + ke) ** 3), 2)
            result["model_used"] = f"3Y Forward EPS - Defensive (g={round(g*100,1)}%, P/E={terminal_pe}x)"
            
        elif book_value_per_share and book_value_per_share > 0:
            intrinsic_value = round(book_value_per_share * 0.80, 2)
            result["model_used"] = "Book Value Haircut (defensive — no FCF/EPS model available)"
            
        else:
            intrinsic_value = round(current_price * 0.75, 2)
            result["model_used"] = "Defensive Haircut (no reliable data for any valuation model)"

    # If no reliable valuation model can run, use the documented defensive current-price haircut instead of implying neutral fair value.
    ...
    entry_low = round(max(support, current_price * 0.85), 2)  # Keep the suggested entry zone from becoming excessively distant from the current market price.
    ...
    stop_loss = round(max(entry_low * 0.80, raw_stop_loss), 2)  # ATR-based stops are padded to avoid normal volatility while maintaining a practical downside boundary.

    # Methodology §12: technicals inform timing rather than the fundamental
    # thesis. Linear drift provides the trend signal; ARIMA is a validation
    # layer when at least 100 observations are available.
    momentum, horizon, drift = "NEUTRAL", "3-5 Years", None
    try:
        closes_clean = hist['Close'].dropna()
        if len(closes_clean) > 30 and current_price:
            # Log-price regression estimates percentage/compound drift rather than raw-price slope, making the trend measure scale-independent.
            log_prices = np.log(closes_clean.values.astype(float))
            if np.all(np.isfinite(log_prices)):
                slope, _ = np.polyfit(np.arange(len(log_prices)), log_prices, 1)
                drift = float(np.exp(slope * 252) - 1)  # annualised compound drift
                if slope > 0.0005:
            ...
            if HAS_ARIMA and len(closes_clean) > 100:
                try:
                    ...
                    # ARIMA is a validation layer; it should not override a non-neutral trend unless the documented agreement rule is satisfied.
                    if arima_dir == momentum:
                        horizon = "12-18 Months (Accelerated)" if momentum == "UP" else "3-5 Years"
                    elif momentum == "NEUTRAL":
                        momentum = arima_dir  # ARIMA casts deciding vote when trend is flat
                        horizon = "12-18 Months (Accelerated)" if momentum == "UP" else "3-5 Years"
                    else:
                        momentum = "MIXED"  # disagreement → neutral mixed signal
...
# 7. MASTER DATA FETCH
# ============================================================
# Data acquisition is intentionally separated from scoring. FMP is preferred
# where available; yfinance fills missing fields so a single provider failure
# does not necessarily prevent analysis. Provider provenance should be made
# explicit in future data-contract work.


# ============================================================
# PRIMARY DATA SOURCE: FMP; SECONDARY FALLBACK: yfinance
# ============================================================
# The methodology requires data provenance and explicit fallback handling.
# This function therefore treats provider choice as an acquisition concern,
# not as part of the valuation calculation itself.
...
def fetch_stock_data(resolved_ticker, raw_input):
    ...
    try:
        ...
        if cf is not None and not cf.empty and 'Free Cash Flow' in cf.index:
            # Remove missing FCF observations before calculating the weighted historical cash-flow input.
    ...
    # This is a discovery aid for weak/uncertain results, not a component of
    # the analysed company's composite score.
            ...
            try:
                ...
                if 0 < pe_val < 30 and roe_val > 15 and dte_val < (2.0 if p_is_fin else 0.8) and is_uptrend:
                    ...
                    if best_peer is None or (roe_val - pe_val) > best_peer.get("_score",-999):
                        best_peer = candidate  # Evaluate the complete peer set so the strongest qualifying alternative is not dependent on iteration order.
            except Exception:
                pass
...
# ============================================================
# 8. UI PLOTLY CHARTS & REPORT COMPONENTS
# ============================================================
# These functions render analysis results. They should not introduce new
# financial logic; presentation belongs downstream of the model engine.
...
# 9. AI NARRATIVE
# ============================================================
# Gemini is a synthesis layer over structured quantitative outputs. It is not
# the source of truth for prices, growth rates, scores, or valuation inputs.
...
# 10. APP STATE & HEADER
# ============================================================
# Streamlit state and application-level controls live here.
...
# ============================================================
# The report consumes the structured model output and narrative. It should
# display model provenance, assumptions, confidence, and fallback status as
# those fields are introduced into the data contract.
